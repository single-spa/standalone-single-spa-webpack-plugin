...
class StandaloneSingleSpaPlugin {
  ...
  modifyScripts({ publicPath, scripts }) {
    if (scripts.modifiedBySingleSpaStandalonePlugin) {
      console.warn(
        "WARNING: You have two separate instances of standalone-single-spa-webpack-plugin in your webpack config. The second instance will be ignored."
      );
      return;
    }
    ...
    if (mainScript.tagName !== "script" || !mainScript.attributes.src) {
      throw Error(
        `standalone-single-spa-webpack-plugin: HtmlWebpackPlugin contains an HTML element that's not a <script> with a 'src' attribute in its scripts array. Found ${mainScript.tagName}. This usually`
      );
    }
